// Command NpmRegistry-cli is a CLI + REPL for the NpmRegistry SDK,
...
const prompt = "npm-registry"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_package search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NPM_REGISTRY_APIKEY (api key), NPM_REGISTRY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NPM_REGISTRY_APIKEY carries the API key and
	// NPM_REGISTRY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NPM_REGISTRY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NPM_REGISTRY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNpmRegistrySDK(opts)
