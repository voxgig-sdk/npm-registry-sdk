...
boru-driven command-line client **and** interactive REPL for the NpmRegistry
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/npm-registry-cli)
make build

# 2. See usage (words, entities, env vars)
./npm-registry-cli --help

# 3. Provide credentials once, via the environment
export NPM_REGISTRY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./npm-registry-cli list get_package
./npm-registry-cli list search

# 5. Override the API base URL for a single call
NPM_REGISTRY_BASE=https://api.example.com ./npm-registry-cli list get_package

# 6. No arguments -> interactive REPL
./npm-registry-cli
npm-registry> list get_package
npm-registry> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/npm-registry-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/npm-registry-cli list get_package
   ```
...
Run with no arguments to open a REPL (prompt `npm-registry>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_package`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/npm-registry-cli`. |
...
(`./npm-registry-cli <expr>`) and interactively (the REPL), and expressions compose the
