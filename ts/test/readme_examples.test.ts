...
//     (\`new NpmRegistrySDK(...)\` / \`NpmRegistrySDK.test(...)\` become
//     \`NpmRegistrySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_package":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NpmRegistrySDK(...)\` and any
// \`NpmRegistrySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NpmRegistrySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NpmRegistrySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
