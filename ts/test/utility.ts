...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NPM_REGISTRY_TEST_LIVE ||
    'TRUE' === process.env.NPM_REGISTRY_TEST_OVERRIDE
  ) {
  ...
  m.NPM_REGISTRY_TEST_EXPLAIN = process.env.NPM_REGISTRY_TEST_EXPLAIN || m.NPM_REGISTRY_TEST_EXPLAIN
