...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NPMREGISTRY_TEST_LIVE ||
    'TRUE' === process.env.NPMREGISTRY_TEST_OVERRIDE
  ) {
  ...
  m.NPMREGISTRY_TEST_EXPLAIN = process.env.NPMREGISTRY_TEST_EXPLAIN || m.NPMREGISTRY_TEST_EXPLAIN
