...
func (sdk *NpmRegistrySDK) OptionsMap() map[string]any {
...
func (sdk *NpmRegistrySDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *NpmRegistrySDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *NpmRegistrySDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *NpmRegistrySDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *NpmRegistrySDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *NpmRegistrySDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("NpmRegistrySDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *NpmRegistrySDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *NpmRegistrySDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("NpmRegistrySDK: graphql: %s", msg)
		res["graphql"] = errors
	}
