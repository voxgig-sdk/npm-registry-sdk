...
class GetPackageEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = NpmRegistrySDK.test(seed, nil)
    seen = base.GetPackage(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.GetPackage(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NPM_REGISTRY_TEST_GET_PACKAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def get_package_basic_setup(extra)
  ...
  entid_env_raw = ENV["NPM_REGISTRY_TEST_GET_PACKAGE_ENTID"]
