...
class GetPackageDirectTest < Minitest::Test
  def test_direct_list_get_package
    setup = get_package_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-get_package", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["get_package01"].each do |_live_key|
...
def get_package_direct_setup(mockres)
