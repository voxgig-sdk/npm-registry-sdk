...
class GetPackageEntity
  ...
  def make
    opts = @_entopts.dup
    GetPackageEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NpmRegistryHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetPackage, Hash] the current GetPackage data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NpmRegistryHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetPackage fields)
  def match_get
  ...
  #
  # @param reqmatch [GetPackageListMatch, Hash, nil] match filter (any subset of
  #   GetPackage fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GetPackage>, Array] the matching GetPackage items; raises NpmRegistryError on failure
