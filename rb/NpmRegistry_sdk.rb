...
require_relative 'NpmRegistry_types'
...
class NpmRegistrySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NpmRegistryHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NpmRegistryHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NpmRegistryFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NpmRegistryUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NpmRegistryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NpmRegistryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NpmRegistryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = NpmRegistryHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NpmRegistryError.new(
        "graphql_error", "NpmRegistrySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NpmRegistrySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
