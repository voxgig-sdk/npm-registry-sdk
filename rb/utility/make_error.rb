...
module NpmRegistryUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = NpmRegistryContext.new({}, nil)
    end
    op = ctx.op || NpmRegistryOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || NpmRegistryResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(NpmRegistryError) ? err.msg : err.to_s
    ...
    sdk_err = NpmRegistryError.new("", msg, ctx)
    sdk_err.result = ctx.utility.clean.call(ctx, result)
    sdk_err.spec = ctx.utility.clean.call(ctx, spec)
    sdk_err.code = err.code if err.is_a?(NpmRegistryError)
