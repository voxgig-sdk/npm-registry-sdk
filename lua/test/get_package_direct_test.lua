...
describe("GetPackageDirect", function()
  it("should direct-list-get_package", function()
    local setup = get_package_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-get_package", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"get_package01"}) do
...
function get_package_direct_setup(mockres)
