...
type Args struct {
	Entity string         `json:"entity" jsonschema:"get_package | search"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.NpmRegistrySDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "npm-registry_list",
		Description: "List records from NpmRegistry. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "npm-registry_load",
		Description: "Load a single record from NpmRegistry. " +
...
func runOp(client *sdk.NpmRegistrySDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.NpmRegistrySDK, name string) (sdk.NpmRegistryEntity, error) {
	switch strings.ToLower(name) {
	case "get_package":
		return client.GetPackage(nil), nil
	case "search":
