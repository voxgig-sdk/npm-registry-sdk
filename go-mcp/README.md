...
[MCP](https://modelcontextprotocol.io) server exposing the NpmRegistry SDK as
two agent tools — `npm-registry_list` and `npm-registry_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/npm-registry-mcp)
make build

# 2. Provide credentials via the environment
export NPM_REGISTRY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user npm-registry \
  -- /absolute/path/to/npm-registry-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./npm-registry-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// npm-registry_list: first page of records
{ "entity": "get_package" }
{ "entity": "get_package", "query": { } }

// npm-registry_load: one record by id
{ "entity": "get_package", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/npm-registry-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user npm-registry \
     -- "$PWD"/dist/*/npm-registry-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `npm-registry_list` and `npm-registry_load` tools now appear
   in new sessions. Ask the agent to *"list get_package using npm-registry"*
   and it calls `npm-registry_list` with `{"entity":"get_package"}`.
...
```sh
export NPM_REGISTRY_APIKEY=sk_live_xxx            # API key
export NPM_REGISTRY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `npm-registry_list` tool
...
### Call the `npm-registry_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `npm-registry_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `npm-registry_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NPM_REGISTRY_APIKEY` | API key sent with every request. |
| `NPM_REGISTRY_BASE` | Optional override of the API base URL. |
...
```sh
./npm-registry-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"npm-registry_load","arguments":{"entity":"get_package","query":{"id":1}}}}'
```
