...
class NpmRegistrySDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = NpmRegistryHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = NpmRegistryHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!NpmRegistryFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, NpmRegistryFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return NpmRegistryUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = NpmRegistryHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = NpmRegistryHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = NpmRegistryHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new NpmRegistryError($op . "_allow",
                "NpmRegistrySDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = NpmRegistryHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new NpmRegistryError("graphql_error",
                "NpmRegistrySDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetPackage($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_package === null) {
                $this->_get_package = new GetPackageEntity($this, null);
            }
            return $this->_get_package;
        }
        return new GetPackageEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new NpmRegistrySDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
