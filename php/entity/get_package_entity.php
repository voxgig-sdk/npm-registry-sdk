...
// NpmRegistry SDK GetPackage entity
...
class GetPackageEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetPackageEntity($this->_client, $opts);
    }

    /**
     * @param GetPackage|array $args GetPackage data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NpmRegistryHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetPackage|array The current GetPackage data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetPackage fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NpmRegistryHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetPackage fields).
     */
    ...
    /**
     * List GetPackage items matching the given filter.
     *
     * @param GetPackageListMatch|array|null $reqmatch Match filter (any subset
     *   of GetPackage fields) as an assoc-array; GetPackageListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetPackage[]|array A list of GetPackage items as assoc-arrays at
     *   the SDK boundary; throws NpmRegistryError on failure (item-5 convention).
     */
