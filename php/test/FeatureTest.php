...
class FtCtrl extends NpmRegistryControl
{
...
class FtClient
{
    ...
    private NpmRegistryUtility $utility;

    public function __construct(NpmRegistryUtility $utility, array $options)
    {
    ...
    public function get_utility(): NpmRegistryUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NpmRegistryContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NpmRegistryContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NpmRegistryContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NpmRegistryConfig::make_feature($name);
    ...
    private static function buildUrl(NpmRegistrySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NpmRegistryError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NpmRegistryResponse($fetched);
        }
    ...
    private function fail(NpmRegistryContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NpmRegistryContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
