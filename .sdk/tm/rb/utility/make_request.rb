...
module NpmRegistryUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NpmRegistryResponse.new({})
    ...
    if fetch_err
      ...
      response = NpmRegistryResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NpmRegistryResponse.new(fetched)
    else
