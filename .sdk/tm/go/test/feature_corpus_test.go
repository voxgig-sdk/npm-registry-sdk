...
//
// NewNpmRegistrySDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.NpmRegistrySDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewNpmRegistrySDK(opts)
}
...
func fcCandidates(client *sdk.NpmRegistrySDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.NpmRegistrySDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.NpmRegistrySDK, name string) any {
